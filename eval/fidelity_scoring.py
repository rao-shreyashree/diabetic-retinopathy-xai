...
usage in Colab notebook:
    ...
    from fidelity_scoring import run_fidelity_scoring
...
usage as a standalone script (after %cd into the eval/ folder in Colab):
    !python fidelity_scoring.py --model resnet50 --method gradcam
"""
...
THRESH_METHOD = "percentile" # "percentile" or "fixed"
THRESH_VALUE  = 75 # top 25% of heatmap counted as "attended"

# core functions
def binarize_heatmap(heatmap: np.ndarray, method: str = "percentile", value: float = 75) -> np.ndarray:
    """Threshold a normalized heatmap into a binary attention mask."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--thresh_value", type=float, default=THRESH_VALUE)
    args = parser.parse_args()

    heatmap_folder = os.path.join(HEATMAP_ROOT, args.model, args.method)
    save_path = os.path.join(
        RESULTS_DIR, f"fidelity_scores_{args.model}_{args.method}.csv"
    )
